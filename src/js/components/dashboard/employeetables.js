            ...
            axios.get(EMPLOYEE_API_URL, {
                ...
                .then(response => {
                    ...
                    if (response.data === "Nothing From AWS Lambda Here") {
                        ...
                        //var m = rfinal.replace(/'/g, '"');
                        //console.log(m)
                        let final = JSON.parse(response.data);
                        console.log(final)
                        this.setState({
                            monthlyinfo: final,
    ...
    render(){
        ...
        if(this.state.monthlyinfo.length===0){
            ...
            for(var i=0; i<this.state.monthlyinfo.length; i++){
                ...
                if(this.state.monthlyinfo[i]['leave']) {
                    monthlyRecordForEmployee.push(
                        <tr key={dayinNumber}>
                            <td>{parseInt(dayinNumber,10)}</td>
                            <td>{this.state.monthlyinfo[i]['Date']}</td>
                            <td style={{color: "#0000ff"}}>OnLeave</td>
                            <td>{this.state.monthlyinfo[i]['entry_time']}</td>
                            <td>{this.state.monthlyinfo[i]['exit_time']}</td>
                            <td>{this.state.monthlyinfo[i]['entrystatus']}</td>
                        </tr>
                    )
                }
                else{
